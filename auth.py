...
# =========================
def _buscar_usuario_por_usuario(db: Session, usuario_usuario: str) -> Optional[models.Usuario]:
    # Si prefieres SQLAlchemy 2.0 puro:
    stmt = select(models.Usuario).where(models.Usuario.usuario_usuario == usuario_usuario)
...
def login_user(request: schemas.LoginRequest, db: Session):
    ...
    return {
        "access_token": token,
        "token_type": "Bearer",
        # Si el front necesita el id aquí, o lo agregas al schema o llama luego a /me
